...
class IDMLPackage(zipfile.ZipFile):
    ...
    def export_as_txt(self):
        ...
        """
        def generate_content_values(data):
            if isinstance(data, dict):
                for key, value in data.items():
                    if key == "text":
                        yield value
                    yield from generate_content_values(value)
            elif isinstance(data, list):
                for item in data:
                    yield from generate_content_values(item)
        ...
        def _export_content_as_tree(xml_structure_node):
            ...
            if len(story_content_and_xmlelement_nodes):
                # Leaf with content.
                if len(xml_structure_node_children) == 0:
                    content.append({"text": "".join([c.text or "" for c in story_content_and_xmlelement_nodes])})  # if not XMLElement
                # Node with content.
                else:
                    xml_structure_child_node = xml_structure_node_children.pop(0)
                    for story_content_node in story_content_and_xmlelement_nodes:
                        if story_content_node.tag == "XMLElement":
                            ...
                            content.append({"text": story_content_node.text})
            else:
                # Node without content > `content' is fed with recursive call on childrens.
                if len(xml_structure_node_children):
                    content.extend(list(map(_export_content_as_tree, xml_structure_node_children)))
        ...
        xml_structure_root_node = self.xml_structure
        return '\n'.join(list(generate_content_values(_export_content_as_tree(xml_structure_root_node))))
    ...
    def _repack(self, target_path, source_dir=None):
        """Re-package the working copy into an IDML file at target_path."""
        source_dir = source_dir or self.working_copy_path
        if not source_dir:
            return

        # Create a new archive from the extracted one.
        # Ensure we don't open self (which might be in read mode) but a new handle
        # But wait, IDMLPackage is a ZipFile.
        # We need to create a NEW zipfile at target_path.
        # Use simple zipfile or IDMLPackage(mode='w')?
        # Usage defines IDMLPackage(mode='w') in decorators.py, let's stick to that but avoid recursion issues if possible.
        # Actually standard zipfile might be safer to avoid init logic, but IDMLPackage init is simple.
        
        # We use ZipFile directly to avoid any side effects of IDMLPackage.__init__ if any
        # although IDMLPackage.__init__ just calls init_lazy_references which is harmless.
        # let's use zipfile.ZipFile to be clean and simple, or IDMLPackage if we want to ensure class consistency.
        # The decorator used IDMLPackage.
        
        with zipfile.ZipFile(target_path, "w", compression=zipfile.ZIP_STORED) as zf:
            for root, dirs, filenames in os.walk(source_dir):
                for filename in filenames:
                    abs_path = os.path.join(root, filename)
                    arcname = abs_path.replace(source_dir, "")
    ...
    def save(self, path=None):
        """Save the IDML package.
        
        If path is provided, save as a new file (Save As). 
        The original file remains identical (unless modifications were already committed).
        If path is None, save changes to the current file (Save).
        """
        if path is None:
            path = self.filename
        
        # If we have a working copy (unsaved changes in a temp dir)
        if self.working_copy_path:
            self._repack(path)
        else:
            # If we are not currently in a working copy context (changes already flushed or just opened)
            # We just copy the file if it's a "Save As"
            if os.path.abspath(path) != os.path.abspath(self.filename):
                shutil.copy2(self.filename, path)
            # If path == filename, nothing to do as changes are presumably already in the file 
            # (or we are in a clean state)
